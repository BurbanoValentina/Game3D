// ══════════════════════════════════════════════════════
//  LEVEL 2 GAME LOOP — Biblioteca de Halliday
//  Same mechanics as Level 1, different puzzle/token data
// ══════════════════════════════════════════════════════
...
export function createGameLoop({ THREE, scene, camera, renderer, player, colliders, worldAssets, inputManager, stateRef, glitchCanvas, glitchCtx, container }) {
  ...
  unsubs.push(EventBus.on('interact', (pos) => {
    LEVEL2_PUZZLES.forEach((p) => {
      ...
      if (hDist < 8 && (!p.position.y || Math.abs(dy) < 8) && !solvedPuzzles.has(p.id)) {
  ...
  unsubs.push(EventBus.on('puzzleSolved', (id) => {
    ...
    const puzzle = LEVEL2_PUZZLES.find(p => p.id === id);
    ...
    if (solvedPuzzles.size >= 5 && s.memoriesFound >= 3) {
      ...
      addLog('>>> LLAVE ÍNDIGO DETECTADA — (0, 4, -80)');
    }
  ...
  const addLog = (msg) => { const logs = store().systemLogs; setState({ systemLogs: [...logs.slice(-8), `[${new Date().toLocaleTimeString()}] ${msg}`] }); };
  ...
  const checkParkourLanding = (px, py, pz) => {
    for (const block of worldAssets.parkourBlocks) {
      ...
      if (px > block.x-halfW && px < block.x+halfW && pz > block.z-halfD && pz < block.z+halfD && py >= topY && py <= topY+2 && player.velocity.y <= 0) return topY + PLAYER_CONFIG.height;
    }
    return null;
  };
  ...
  const loop = () => {
    ...
    if (parkourY !== null && player.velocity.y <= 0) { player.position.y = parkourY; player.velocity.y = 0; player.grounded = true; }
    ...
    if (memoryEchoActive) {
      memoryEchoTimer--; setState({ memoryTimer: Math.ceil(memoryEchoTimer/60) });
    ...
    // Animations
    worldAssets.neonLights.forEach(nl => { nl.light.intensity = nl.baseIntensity * (0.4 + 0.6 * Math.sin(time * 1.5 + nl.phase)); });
    const posArr = worldAssets.particles.geometry.attributes.position.array;
    for (let i = 0; i < posArr.length/3; i++) { posArr[i*3+1] += Math.sin(time+i)*0.006; if(posArr[i*3+1]>50) posArr[i*3+1]=0; }
    ...
    const s = store();
    if(solvedPuzzles.size>=5&&s.memoriesFound>=3){worldAssets.keyMesh.rotation.x=time*0.5;worldAssets.keyMesh.rotation.y=time*0.8;worldAssets.keyMesh.position.y=4+Math.sin(time*1.5)*0.8;}
    worldAssets.npcs.forEach(npc => { npc.mesh.position.x = npc.baseX+Math.sin(time*0.5+npc.loopPhase)*2; npc.mesh.position.z = npc.baseZ+Math.cos(time*0.5+npc.loopPhase)*2; npc.mesh.material.opacity = 0.3+Math.sin(time*10+npc.loopPhase)*0.2; });
    worldAssets.checkpointMeshes.forEach(cp => {
      const dx = player.position.x-cp.x, dz = player.position.z-cp.z;
      if(Math.sqrt(dx*dx+dz*dz)<3 && !cp.reached){ cp.reached=true; cp.mesh.material.color.set(0xffbb33); cp.mesh.material.opacity=0.5; setState({checkpointsReached:[...store().checkpointsReached,cp.label]}); addLog(`CHECKPOINT: ${cp.label}`); }
    });
    let nearSomething = false;
    LEVEL2_PUZZLES.forEach(p => { const dx=player.position.x-p.position.x,dz=player.position.z-p.position.z; if(Math.sqrt(dx*dx+dz*dz)<7&&!solvedPuzzles.has(p.id)){nearSomething=true;setState({interactionPrompt:`[E] ${p.title}`});}});
    LEVEL2_TOKENS.forEach(t => { const dx=player.position.x-t.position.x,dz=player.position.z-t.position.z; if(Math.sqrt(dx*dx+dz*dz)<5&&!collectedTokenIds.has(t.id)){nearSomething=true;setState({interactionPrompt:t.type==='memory'?'🔮 [E] Recuerdo':'❓ [E] Token'});}});
    if(solvedPuzzles.size>=5&&s.memoriesFound>=3){const dkx=player.position.x,dkz=player.position.z+80;if(Math.sqrt(dkx*dkx+dkz*dkz)<7){nearSomething=true;setState({interactionPrompt:'[E] LLAVE ÍNDIGO'});}}
    if(!nearSomething) setState({interactionPrompt:''});
    if(!memoryEchoActive&&Math.random()<0.001){const ghost=worldAssets.memoryGhost;ghost.mesh.material.opacity=0.12;ghost.mesh.position.set(player.position.x+(Math.random()-0.5)*20,1.75,player.position.z+(Math.random()-0.5)*20);setTimeout(()=>{ghost.mesh.material.opacity=0;},2000);}
    ...
    glitchCtx.fillStyle='rgba(42,24,16,0.01)';
