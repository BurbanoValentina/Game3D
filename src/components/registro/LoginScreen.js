...
export default function LoginScreen() {
  ...
  const handleSubmit = async () => {
    if (!validate()) return;
    setLoading(true); setServerMsg('');
    try {
      const res = await fetch('/api/auth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'login', email: email.trim(), password }) });
