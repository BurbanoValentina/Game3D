...
export function createGameLoop({ THREE, scene, camera, renderer, player, colliders, worldAssets, inputManager, stateRef, glitchCanvas, glitchCtx, container }) {
  ...
  unsubs.push(EventBus.on('puzzleSolved', (id) => {
    ...
    const puzzle = LEVEL5_PUZZLES.find(p => p.id === id);
  ...
  const loop = () => {
    ...
    let near=false;
    LEVEL5_PUZZLES.forEach(p=>{const dx=player.position.x-p.position.x,dz=player.position.z-p.position.z;if(Math.sqrt(dx*dx+dz*dz)<7&&!solvedPuzzles.has(p.id)){near=true;setState({interactionPrompt:'[E] '+p.title});}});
    LEVEL5_TOKENS.forEach(t=>{const dx=player.position.x-t.position.x,dz=player.position.z-t.position.z;if(Math.sqrt(dx*dx+dz*dz)<5&&!collectedTokenIds.has(t.id)){near=true;setState({interactionPrompt:t.type==='memory'?'🔮 [E] Recuerdo':'❓ [E] Token'});}});
    if(solvedPuzzles.size>=5&&st.memoriesFound>=3){const dkx=player.position.x,dkz=player.position.z-(-80);if(Math.sqrt(dkx*dkx+dkz*dkz)<7){near=true;setState({interactionPrompt:'[E] LLAVE BLANCA'});}}
