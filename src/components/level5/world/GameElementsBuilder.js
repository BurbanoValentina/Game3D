// ══════════════════════════════════════════════════════
//  LEVEL 5 GAME ELEMENTS — Corazón del OASIS
//  White/prismatic puzzles, tokens, parkour, final key
// ══════════════════════════════════════════════════════

import { LEVEL5_PUZZLES, LEVEL5_CHECKPOINTS, LEVEL5_TOKENS, LEVEL5_PARKOUR } from '../../../constants/level5Constants';

const CORE_COLORS = [0xFFFFFF, 0xFFBB33, 0x4B0082, 0xDC143C, 0x00FF88, 0x9D00FF, 0xFF0066, 0x00f0ff];

export function buildPuzzleMarkers(scene, THREE, assets) {
  LEVEL5_PUZZLES.forEach((p) => {
    const py = p.position.y || 0;
    const pillar = new THREE.Mesh(new THREE.CylinderGeometry(0.5,0.7,4,6), new THREE.MeshPhongMaterial({color:0xFFFFFF,emissive:0x888888,transparent:true,opacity:0.15,wireframe:true}));
    pillar.position.set(p.position.x, py+2, p.position.z); scene.add(pillar);
    const cube = new THREE.Mesh(new THREE.OctahedronGeometry(1.0), new THREE.MeshPhongMaterial({color:0xFFFFFF,emissive:0xCCCCCC,transparent:true,opacity:0.85,shininess:120}));
    cube.position.set(p.position.x, py+5, p.position.z); scene.add(cube);
    const ring = new THREE.Mesh(new THREE.TorusGeometry(2,0.06,8,32), new THREE.MeshBasicMaterial({color:0xFFFFFF,transparent:true,opacity:0.25}));
    ring.position.set(p.position.x,py+0.3,p.position.z); ring.rotation.x=-Math.PI/2; scene.add(ring);
    const cubeLight = new THREE.PointLight(0xFFFFFF, 5, 30); cubeLight.position.copy(cube.position); scene.add(cubeLight);
    const beamH=140;
    const beam=new THREE.Mesh(new THREE.CylinderGeometry(0.3,1.5,beamH,8,1,true),new THREE.MeshBasicMaterial({color:0xFFFFFF,transparent:true,opacity:0.06,side:THREE.DoubleSide}));
    beam.position.set(p.position.x,beamH/2,p.position.z); scene.add(beam);
    const core=new THREE.Mesh(new THREE.CylinderGeometry(0.1,0.5,beamH,6,1,true),new THREE.MeshBasicMaterial({color:0xFFDDDD,transparent:true,opacity:0.12,side:THREE.DoubleSide}));
...
export function buildTokenMarkers(scene, THREE, assets) {
  LEVEL5_TOKENS.forEach((token) => {
    const tc = token.type==='memory'?0xFFD700:0xFFFFFF;
...
export function buildParkourBlocks(scene, THREE, assets) {
  LEVEL5_PARKOUR.forEach((parkour) => {
    const pc = CORE_COLORS[parkour.puzzleId % CORE_COLORS.length];
    parkour.blocks.forEach((block) => {
      ...
      const x = c.getContext('2d'); x.fillStyle='rgba(10,10,20,0.9)'; x.fillRect(0,0,64,64);
      ...
      x.globalAlpha=1;
      const mat = new THREE.MeshPhongMaterial({map:new THREE.CanvasTexture(c),color:0x1A1A30,emissive:new THREE.Color(pc).multiplyScalar(0.08),transparent:true,opacity:0.9,shininess:80});
...
export function buildKeyAndCheckpoints(scene, THREE, assets) {
  ...
  assets.keyMat = new THREE.MeshPhongMaterial({color:0xFFFFFF,emissive:0xCCCCCC,transparent:true,opacity:0,shininess:120});
  assets.keyMesh = new THREE.Mesh(keyGeo, assets.keyMat);
  assets.keyMesh.position.set(0,4,-80); scene.add(assets.keyMesh);
  assets.keyLight = new THREE.PointLight(0xFFFFFF,0,35);
  assets.keyLight.position.set(0,4,-80); scene.add(assets.keyLight);

  LEVEL5_CHECKPOINTS.forEach((cp) => {
    const ring = new THREE.Mesh(new THREE.RingGeometry(2,2.3,6), new THREE.MeshBasicMaterial({color:0xFFFFFF,transparent:true,opacity:0.15,side:THREE.DoubleSide}));
    ring.rotation.x=-Math.PI/2; ring.position.set(cp.x,0.05,cp.z); scene.add(ring);
    const inner = new THREE.Mesh(new THREE.RingGeometry(0.8,1,6), new THREE.MeshBasicMaterial({color:0xFFFFFF,transparent:true,opacity:0.08,side:THREE.DoubleSide}));
