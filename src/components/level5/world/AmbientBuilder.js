// ══════════════════════════════════════════════════════
//  LEVEL 5 AMBIENT — Corazón del OASIS
// ══════════════════════════════════════════════════════
import { LEVEL5_GRAFFITI } from '../../../constants/level5Constants';
const LVL_COLORS = [0xFFFFFF, 0xFFBB33, 0x4B0082, 0xDC143C, 0x00FF88];
...
export function buildGraffiti(scene, THREE, assets, buildings) {
  const data=LEVEL5_GRAFFITI; const count=Math.min(buildings.length,data.length);
  for(let i=0;i<count;i++){const b=buildings[i],g=data[i],side=Math.random()>0.5?1:-1;const cv=document.createElement('canvas');cv.width=512;cv.height=128;const ctx=cv.getContext('2d');ctx.fillStyle='rgba(10,10,20,0.6)';ctx.fillRect(0,20,512,80);ctx.strokeStyle=g.color;ctx.globalAlpha=0.3;ctx.lineWidth=1;ctx.strokeRect(4,22,504,76);ctx.globalAlpha=1;ctx.font='bold 32px monospace';ctx.fillStyle=g.color;ctx.shadowColor=g.color;ctx.shadowBlur=20;ctx.fillText(g.text,16,68);ctx.shadowBlur=0;const mat=new THREE.MeshBasicMaterial({map:new THREE.CanvasTexture(cv),transparent:true,side:THREE.DoubleSide,opacity:0.85});const mesh=new THREE.Mesh(new THREE.PlaneGeometry(8,2),mat);mesh.position.set(b.mesh.position.x+side*(b.width/2+0.1),5+Math.random()*8,b.mesh.position.z);mesh.rotation.y=side>0?-Math.PI/2:Math.PI/2;scene.add(mesh);assets.graffitiMeshes.push(mesh);}
}
...
export function buildGhost(scene, THREE, assets) {
  const ghost=new THREE.Mesh(new THREE.CylinderGeometry(0.4,0.6,3.5,6),new THREE.MeshPhongMaterial({color:0xFFFFFF,emissive:0xFFFFFF,transparent:true,opacity:0,wireframe:true}));ghost.position.set(0,1.75,0);scene.add(ghost);
  ...
  const tg=new THREE.BufferGeometry();tg.setAttribute('position',new THREE.BufferAttribute(tp,3));scene.add(new THREE.Points(tg,new THREE.PointsMaterial({color:0xFFFFFF,size:0.15,transparent:true,opacity:0})));
  assets.memoryGhost={mesh:ghost,visible:false,timer:0};
}
export function buildBillboards(scene, THREE) {
  const texts=['ZONA CERO','HALLIDAY.SYS','EVA_∞','EL NÚCLEO'];
  for(let i=0;i<4;i++){const c=document.createElement('canvas');c.width=320;c.height=180;const x=c.getContext('2d');x.fillStyle='rgba(10,10,20,0.8)';x.fillRect(0,0,320,180);const bc=['#FFFFFF','#FFD700','#9D00FF','#00f0ff'][i];x.strokeStyle=bc;x.globalAlpha=0.3;x.lineWidth=2;x.strokeRect(4,4,312,172);x.globalAlpha=1;x.font='bold 24px monospace';x.fillStyle=bc;x.shadowColor=bc;x.shadowBlur=15;x.fillText(texts[i],20,80);x.shadowBlur=0;const bill=new THREE.Mesh(new THREE.PlaneGeometry(7,4.5),new THREE.MeshBasicMaterial({map:new THREE.CanvasTexture(c),transparent:true,side:THREE.DoubleSide,opacity:0.7}));bill.position.set((Math.random()-0.5)*170,18+Math.random()*20,(Math.random()-0.5)*170);bill.rotation.y=Math.random()*Math.PI;scene.add(bill);}
}
