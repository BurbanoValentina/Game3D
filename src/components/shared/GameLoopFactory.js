// ══════════════════════════════════════════════════════
//  SHARED GAME LOOP FACTORY — Used by all levels
//  Takes level-specific puzzles, tokens, key position
// ══════════════════════════════════════════════════════
...
export function createGameLoopFactory(levelData) {
  const { puzzles, tokens, keyPosition } = levelData;

  return function createGameLoop({
    ...
    const checkStartTimer = () => {
      if (!levelTimerInterval && stateRef.current === GameStates.PLAYING) startLevelTimer();
    };
    ...
    unsubs.push(EventBus.on('interact', (pos) => {
      ...
      if (solvedPuzzles.size >= 5 && s.memoriesFound >= 3) {
        const dkx = pos.x - keyPosition.x, dkz = pos.z - keyPosition.z;
    ...
    unsubs.push(EventBus.on('puzzleSolved', (id) => {
      ...
      if (solvedPuzzles.size >= 5 && s.memoriesFound >= 3) {
        ...
        addLog(`>>> LLAVE DETECTADA — (${keyPosition.x}, 4, ${keyPosition.z})`);
      }
    ...
    const loop = () => {
      ...
      worldAssets.checkpointMeshes.forEach(cp => {
        ...
        if (Math.sqrt(dx * dx + dz * dz) < 3 && !cp.reached) {
          cp.reached = true; cp.mesh.material.color.set(0xFFD700); cp.mesh.material.opacity = 0.5;
      ...
      if (solvedPuzzles.size >= 5 && s.memoriesFound >= 3) {
        const dkx = player.position.x - keyPosition.x, dkz = player.position.z - keyPosition.z;
        if (Math.sqrt(dkx * dkx + dkz * dkz) < 7) { nearSomething = true; setState({ interactionPrompt: `[E] LLAVE` }); }
      }
