// ══════════════════════════════════════════════════════
//  LEVEL 4 GAME ELEMENTS — Puente Suspendido
//  Emerald-themed puzzles, tokens, parkour, key
// ══════════════════════════════════════════════════════

import { LEVEL4_PUZZLES, LEVEL4_CHECKPOINTS, LEVEL4_TOKENS, LEVEL4_PARKOUR } from '../../../constants/level4Constants';

const BRIDGE_COLORS = [0x00FF88, 0x61FFD8, 0x00f0ff, 0x00AA55, 0xFFD700, 0x9D00FF, 0xFF61D8, 0xD861FF];

export function buildPuzzleMarkers(scene, THREE, assets) {
  LEVEL4_PUZZLES.forEach((p) => {
    const py = p.position.y || 0;
    const pillar = new THREE.Mesh(new THREE.CylinderGeometry(0.5,0.7,4,6), new THREE.MeshPhongMaterial({color:0x00FF88,emissive:0x005533,transparent:true,opacity:0.15,wireframe:true}));
    pillar.position.set(p.position.x, py+2, p.position.z); scene.add(pillar);
    const cube = new THREE.Mesh(new THREE.OctahedronGeometry(1.0), new THREE.MeshPhongMaterial({color:0x61FFD8,emissive:0x00FF88,transparent:true,opacity:0.85,shininess:100}));
    cube.position.set(p.position.x, py+5, p.position.z); scene.add(cube);
    const ring = new THREE.Mesh(new THREE.TorusGeometry(2,0.06,8,32), new THREE.MeshBasicMaterial({color:0x00FF88,transparent:true,opacity:0.25}));
    ring.position.set(p.position.x,py+0.3,p.position.z); ring.rotation.x=-Math.PI/2; scene.add(ring);
    const cubeLight = new THREE.PointLight(0x00FF88, 5, 30); cubeLight.position.copy(cube.position); scene.add(cubeLight);
    const beamH=140;
    const beam=new THREE.Mesh(new THREE.CylinderGeometry(0.3,1.5,beamH,8,1,true),new THREE.MeshBasicMaterial({color:0x00FF88,transparent:true,opacity:0.06,side:THREE.DoubleSide}));
    beam.position.set(p.position.x,beamH/2,p.position.z); scene.add(beam);
    const core=new THREE.Mesh(new THREE.CylinderGeometry(0.1,0.5,beamH,6,1,true),new THREE.MeshBasicMaterial({color:0x61FFD8,transparent:true,opacity:0.12,side:THREE.DoubleSide}));
...
export function buildTokenMarkers(scene, THREE, assets) {
  LEVEL4_TOKENS.forEach((token) => {
    const tc = token.type==='memory'?0xFFD700:0x00FF88;
...
export function buildParkourBlocks(scene, THREE, assets) {
  LEVEL4_PARKOUR.forEach((parkour) => {
    const pc = BRIDGE_COLORS[parkour.puzzleId % BRIDGE_COLORS.length];
    parkour.blocks.forEach((block) => {
      ...
      const x = c.getContext('2d'); x.fillStyle='rgba(5,15,10,0.9)'; x.fillRect(0,0,64,64);
      ...
      x.globalAlpha=1;
      const mat = new THREE.MeshPhongMaterial({map:new THREE.CanvasTexture(c),color:0x051510,emissive:new THREE.Color(pc).multiplyScalar(0.08),transparent:true,opacity:0.9,shininess:80});
...
export function buildKeyAndCheckpoints(scene, THREE, assets) {
  ...
  assets.keyMat = new THREE.MeshPhongMaterial({color:0x00FF88,emissive:0x00AA55,transparent:true,opacity:0,shininess:100});
  assets.keyMesh = new THREE.Mesh(keyGeo, assets.keyMat);
  assets.keyMesh.position.set(0,4,-80); scene.add(assets.keyMesh);
  assets.keyLight = new THREE.PointLight(0x00FF88,0,35);
  assets.keyLight.position.set(0,4,-80); scene.add(assets.keyLight);

  LEVEL4_CHECKPOINTS.forEach((cp) => {
    const ring = new THREE.Mesh(new THREE.RingGeometry(2,2.3,6), new THREE.MeshBasicMaterial({color:0x00FF88,transparent:true,opacity:0.15,side:THREE.DoubleSide}));
    ring.rotation.x=-Math.PI/2; ring.position.set(cp.x,0.05,cp.z); scene.add(ring);
    const inner = new THREE.Mesh(new THREE.RingGeometry(0.8,1,6), new THREE.MeshBasicMaterial({color:0x00FF88,transparent:true,opacity:0.08,side:THREE.DoubleSide}));
